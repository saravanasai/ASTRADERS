<?php 
  include("../../config.php");
    ...
    { 
     ...
     $sql="SELECT *,SUM(`TR_AMOUNT_PAID`) AS TOTAL_ON_DAY FROM `todayTransactionView` WHERE `TR_DONE_ON`=:ag_id AND `TR_COMMIT_STATUS`=0 AND `TR_DATE` BETWEEN :fromdate AND :todate GROUP BY `TR_DATE` ORDER BY `todayTransactionView`.`TR_DATE` ASC" ;
